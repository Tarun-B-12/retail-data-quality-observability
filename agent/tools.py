...
from src.generate_report import generate_trend_chart, generate_html_report
import json

TOOL_DEFINITIONS = [
    {
        "name": "load_data",
        "description": "Load the retail dataset from the raw data folder.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Run the validation suite. Returns pass/fail results and health score.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "store_results",
        "description": "Store validation results into DuckDB for historical tracking.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "get_run_history",
        "description": "Retrieve all past validation runs and health score trends.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "explain_failures",
        "description": "Use an LLM to generate plain-English explanations for failed checks.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "save_report",
        "description": "Save anomaly explanations to a markdown report file.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "generate_trend_chart",
        "description": "Generate a health score trend chart PNG from run history.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    },
    {
        "name": "generate_html_report",
        "description": "Generate a full HTML quality report with score cards, validation table, and trend chart.",
        "input_schema": {"type": "object", "properties": {}, "required": []}
    }
]
...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    if tool_name == "load_data":
        ...
        _state["df"] = df
        return json.dumps({"status": "success", "rows_loaded": len(df), "columns": list(df.columns)})
        ...
        if _state["validation_results"] is None:
            return json.dumps({"error": "No validation results."})
        ...
        return json.dumps({
            ...
            "preview": results["explanations"][0]["explanation"][:200] if results["explanations"] else "No failures"
        })

    elif tool_name == "save_report":
        if _state["explanation_results"] is None:
            return json.dumps({"error": "No explanations found."})
